...
function Invoke-DpiSuite {
    ...
    Write-Host "[INFO] Starting DPI TCP 16-20 checks (parallel: $MaxParallel)..." -ForegroundColor Red
...
function Restore-WinwsSnapshot {
    ...
    Write-Host "[INFO] Restoring previously running winws instances..." -ForegroundColor Red
...
try {
    ...
    if ($testType -eq 'standard') {
        ...
        Write-Host $script:currentLine -ForegroundColor Red
        ...
        foreach ($target in $targetList) {
            ...
            if ($res.IsUrl -and $res.HttpTokens) {
                ...
                Write-Host " | Ping: " -NoNewline -ForegroundColor Red
        ...
        Write-Host "  > Running DPI checkers..." -ForegroundColor Red
    ...
    if ($originalIpsetStatus -ne "any") {
        Write-Host "[INFO] Restoring original ipset mode..." -ForegroundColor Red
        Set-IpsetMode -mode "restore"
    }
    Remove-Item -Path $ipsetFlagFile -ErrorAction SilentlyContinue
}
